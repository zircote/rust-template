...
description: "How a change becomes a signed, independently verifiable release via the zircote/.github central reusables."
sidebar:
...
> [`CI-WORKFLOWS.md`](/rust-template/workflows/ci-workflows/) and
> [`SIGNED-RELEASES.md`](/rust-template/security/signed-releases/). This document explains how the
> pieces compose.
...
> [`SIGNED-RELEASES.md` § Container Image Attestations](/rust-template/security/signed-releases/#container-image-attestations).
...
5. **(Optional) Other secrets.** `CODECOV_TOKEN` (coverage upload),
   ...
   delivery. See the
   [Required Secrets Summary](/rust-template/workflows/ci-workflows/#required-secrets-summary).
...
- [`SIGNED-RELEASES.md`](/rust-template/security/signed-releases/) — verification commands, SLSA
  levels, keyless Sigstore, "who signs what", troubleshooting.
- [`CI-WORKFLOWS.md`](/rust-template/workflows/ci-workflows/) — per-workflow reference:
